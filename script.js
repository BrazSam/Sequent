...
function renderStats() {
  ...
  const semContato = students.filter(x => x.status === 'sem-contato').length;
  const rematric   = students.filter(x => x.status === 'rematriculou').length;
  const urgentes   = students.filter(x => daysUntil(x.termino) <= 7).length;
